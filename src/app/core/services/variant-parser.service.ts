...
import { OptionValue, Variant, OptionType } from '../../products/product';
...
export class VariantParserService {
  ...
  variantIdsMaker(optionValue: OptionValue, optionTypesHash: OptionTypesHash,
    optionType: OptionType, variant: Variant) {
    const correspondingOptionValues = this.getOtherOptionValues(optionValue, optionType);
    if (optionTypesHash[optionType.name] != null && optionTypesHash[optionType.name][optionValue.name] != null) {
      ...
      variantArr.push({[variant._id]: correspondingOptionValues });
      return variantArr;
    } else {
      return Array.of({[variant._id]: correspondingOptionValues});
    }
  }
  ...
  getOtherOptionValues(optionValue, currOptionType) {
    ...
    for (let i = 0; i < this.currVariantOptionValues.length; i++) {
      if (this.currVariantOptionValues[i].optionTypeName !== currOptionType.name) {
        correspondingOptionValues.push({ [this.currVariantOptionValues[i].optionTypeName]: this.currVariantOptionValues[i].name });
      }
    }
